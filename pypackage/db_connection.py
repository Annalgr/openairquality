...
def check_db(args):            
    # Open connection to the regisered users db
    conn = sqlite3.connect('pypackage/openaq_users.db')
    cursor = conn.cursor()    

    # Check for username
    row = cursor.execute("SELECT * FROM user_database WHERE username = ?",(args.username,))
    ...
    if results:   
        ...
        if digest == results[0][1]:
            ...
            return print("Password is invalid for user {}.".format(args.username))            
    else:
